...
The root cause was command ordering between the VS Code editor save lifecycle and terminal command submission. Browser e2e also showed a Cargo freshness edge: after the file was mirrored correctly, an immediate `cargo run --release` could still execute the pre-baked binary when the source write landed in the same one-second mtime bucket as Cargo's existing target fingerprints.
...
Guest writes for Rust/Cargo inputs now also bump the source mtime past Cargo's one-second freshness boundary when `target/` exists. This prevents an immediate browser-side save followed by `cargo run --release` from reusing the pre-baked binary.
