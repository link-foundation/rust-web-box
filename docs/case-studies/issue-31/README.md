...
| `verification/webvm-server-after-fingerprint-marker-fix.log` | Focused WebVM server tests passing after the save-script fingerprint marker guard was added. |
| `verification/focused-node-after-fingerprint-marker-fix.log` | Focused boot-shell, disk-staging, and WebVM server tests passing after the CI e2e stale-binary failure was fixed. |
| `verification/node-web-tests-after-fingerprint-marker-fix.log` | Full web test suite passing after the fingerprint marker guard was added. |
...
- Browser saves now age existing `/workspace/target` metadata after writing Cargo inputs and overwrite existing Cargo text fingerprint hash files with a stale hash-shaped marker while leaving binary dep-info files intact. That forces Cargo's normal rebuild path from the edited source without deleting fingerprint trees; the fingerprint backstop covers CheerpX lower-layer delete behavior and ext2 mtime cases where metadata aging alone can still leave Cargo trusting the pre-baked binary.
...
- `node --test web/tests`
- `node --test web/tests/boot-shell.test.mjs web/tests/webvm-server.test.mjs web/tests/disk-staging.test.mjs`
- `cargo fmt --all -- --check`
